...
class ChessController {
    ...
    removePieceEvent = (pieceToRemove) => { // callback to for the model remove a piece from board (view)
        if (!isNull(pieceToRemove))
            this.GameView.removePieceFromBoard(pieceToRemove)
        else
            console.log("Trying to remove from view a piece that doesnt exist in the model");
    }

    mouseDownOnPieceEvent = (whereX, whereY) => {
        if (!isNull(this.GameData.chessBoard[whereX][whereY])) {
            this.ongoingPossibleMoves = this.GameData.getPossibleMoves(whereX, whereY);
            if (this.ongoingPossibleMoves.length > 0) {
                console.log(whereX, whereY); // piece is raised !!
                let strCursor = this.GameData.chessBoard[whereX][whereY].getPieceCursorUrl();
    ...
    releasedPieceEvent = (whereX, whereY) => {
        ...
        if (bWillMoveThere){
            let moveNotation = this.GameData.movePiece(lastX, lastY, whereX, whereY,this.removePieceEvent);
            this.GameView.addMoveToList(moveNotation);
            this.GameView.putPieceOnBoard(this.GameData.chessBoard[whereX][whereY], this.mouseDownOnPieceEvent);
        } else // released somewhere else !!
        {
            if (!isNull(lastX) &&
                !isNull(lastY) &&
                !isNull(this.GameData.chessBoard[lastX][lastY]))
            // put back the piece
                this.GameView.putPieceOnBoard(this.GameData.chessBoard[lastX][lastY], this.mouseDownOnPieceEvent);
            else
                throw "Release mouse without press ?!!";
        }
        this.whereWasPieceRaisedFromX = null;
        this.whereWasPieceRaisedFromY = null;
        this.ongoingPossibleMoves = null;
        ...
        for (let row = 0; row < BOARD_SIZE; row++)
            for (let col = 0; col < BOARD_SIZE; col++)
                if (!isNull(this.GameData.chessBoard[row][col])) {
                    this.GameView.putPieceOnBoard(this.GameData.chessBoard[row][col], this.mouseDownOnPieceEvent);
                }
    }
}
