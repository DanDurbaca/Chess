...
class Pawn extends ChessPiece {
    ...
    getMoveNotation(whereX,whereY)
    {
        ...
        if (!isNull(whereX) && !isNull(whereY) &&
            !isNull(this.RowPos) && !isNull(this.ColPos)){
            if (whereY!=this.ColPos)
                strRetVal +=  String.fromCharCode(ALPHA+this.ColPos) + "x";
            strRetVal +=  String.fromCharCode(ALPHA+whereY) + (BOARD_SIZE-whereX).toString();
            }
        return strRetVal;
    }
    ...
    movePiece(whereX,whereY,controllerHandler)
    {
        ...
        if (Math.abs(oldRow-whereX)==2) // set the ghostPawnPos
        {
            if (oldRow>whereX)
                Pawn.ghostPawn = new Array(oldRow-1,oldCol);
            else
                Pawn.ghostPawn = new Array(oldRow+1,oldCol);
    ...
    getPossibleMoves() {
        ...
        if (targetRow > 0 && targetRow < BOARD_SIZE - 1) {
            ...
            for (let p of tryouts)
            {
                ...
                if (this.ColPos + p >= 0 &&
                    this.ColPos + p < BOARD_SIZE)
                    if(!isNull(fullBoard[targetRow][this.ColPos + p]) &&
                       this.PieceColor != fullBoard[targetRow][this.ColPos + p].PieceColor)
                        arrRetVals.push(targetPos); // capture possible here
                    else
                    if (!isNull(Pawn.ghostPawn)&&
        ...
        if (this.PieceColor == PieceColors.White &&
